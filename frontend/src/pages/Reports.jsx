...
export default function Reports() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-100">Reports Module</h1>
          ...
          <Card key={report.key} title={report.title}>
            <div className="space-y-3">
              ...
              <div className="max-h-64 overflow-auto border border-slate-800 rounded-lg p-3 bg-slate-900/60">
                <pre className="text-xs text-slate-400 whitespace-pre-wrap">{JSON.stringify(report.rows.slice(0, 5), null, 2)}</pre>
              </div>
            </div>
          </Card>
        ))}
