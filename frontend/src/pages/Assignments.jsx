...
export default function Assignments() {
  ...
  const handleAssign = async (e) => {
    ...
    try {
      ...
      window.alert('Failed to assign worker: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-100">Worker Assignment</h1>
            ...
            <tbody>
              {filteredAssignments.map((assignment) => (
                <tr key={assignment.id} className="border-b border-slate-800">
                  <td className="py-3 px-4 text-slate-100">{getWorkerName(assignment.workerId)}</td>
