            ...
            <tbody>
              {filteredInventory.length > 0 ? (
                filteredInventory.map(item => (
                  <tr
                    key={item.id}
                    className={`border-b border-slate-800 hover:bg-slate-800/40/50 transition-colors ${
                      isLowStock(item) ? 'bg-rose-500/10' : ''
                    }`}
                  >
                    <td className="py-3 px-4 text-slate-100">{item.item_name}</td>
                    <td className="py-3 px-4 text-slate-400">{item.category}</td>
                    <td className="py-3 px-4 text-slate-100">{formatCurrency(item.unit_cost)}</td>
                    <td className="py-3 px-4 text-slate-100">{item.current_stock} {item.uom}</td>
                    <td className="py-3 px-4 text-slate-100">{item.min_stock_qty} {item.uom}</td>
              ...
              <div className="space-y-2">
                {inventory.filter(isLowStock).map(item => (
                  <div key={item.id} className="flex justify-between items-center p-3 bg-slate-800/50 rounded-lg">
                    <div>
                      <p className="text-slate-100">{item.item_name}</p>
      ...
      <Modal isOpen={!!addStockModal} onClose={() => setAddStockModal(null)} title="Add Stock">
        <div className="space-y-4">
          <p className="text-slate-400 text-sm">
            Adding stock for: <span className="text-slate-100 font-medium">
              {inventory.find((i) => String(i.id) === String(addStockModal))?.item_name}
            </span>
          </p>
          <Input
