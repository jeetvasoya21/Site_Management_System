...
export default function Procurement() {
  ...
  const handleMarkDelivered = async (poId) => {
    setUpdatingId(poId);
    try {
      await updatePurchaseDeliveryStatus(poId, 'delivered');
    } catch (err) {
      window.alert('Failed to mark as delivered: ' + err.message);
    } finally {
      setUpdatingId(null);
    }
  };
  ...
  const getProjectName = (id) => projects.find((p) => String(p.id) === String(id))?.project_name || String(id);
  const getVendorName = (id) => vendors.find((v) => String(v.id) === String(id))?.vendor_name || String(id);
  const getItemName = (id) => inventory.find((i) => String(i.id) === String(id))?.item_name || String(id);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-100">Procurement Management</h1>
      ...
      <Card title="Create Purchase Order">
        <form className="grid grid-cols-1 md:grid-cols-3 gap-4" onSubmit={handleSubmit}>
          ...
          <Select
            ...
            onChange={(e) => {
              const item = inventory.find((inv) => String(inv.id) === String(e.target.value));
            ...
            <tbody>
              {filteredOrders.map((po) => (
                <tr key={po.id} className="border-b border-slate-800">
                  <td className="py-3 px-4 text-slate-100">{po.procurement_id || `PO-${po.id}`}</td>
                  ...
                  <td className="py-3 px-4 text-slate-100">{formatCurrency(Number(po.quantity) * Number(po.unit_price))}</td>
                  ...
                  <td className="py-3 px-4">
                    {po.delivery_status !== 'delivered' && (
                      ...
                      >
                        {updatingId === po.id ? 'Updating...' : 'Mark Delivered'}
                      </Button>
                    )}
                  </td>
                </tr>
