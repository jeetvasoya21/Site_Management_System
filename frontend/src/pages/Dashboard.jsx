...
const Dashboard = () => {
  ...
  const kpis = useMemo(() => {
    ...
    if (user?.role === 'Project_Manager') {
      ...
      // Site Engineer sees assigned tasks only — compare as strings
      filteredTasks = tasks.filter(t => String(t.assigned_to) === String(user?.id));
    }
    ...
    const activeTasks = filteredTasks.filter(t => t.status === 'In Progress' || t.status === 'In_Progress').length;
  ...
  const budgetChartData = useMemo(() => {
    return projects.map((project) => {
      const projectFinance = financeRecords.filter(
        (f) => String(f.projectId) === String(project.id)
      );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button className="px-4 py-2 bg-slate-900 border border-slate-800 text-slate-300 rounded-lg hover:bg-slate-800/40 text-sm font-medium shadow-sm transition-all focus:ring-2 focus:ring-primary-500/20">
            Export Report
          </button>
          {user?.role === 'Admin' && (
      ...
      <Card>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm text-slate-400 mb-2">Global Search</label>
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-900 border border-slate-800">
              <Search size={16} className="text-slate-400" />
              <input
                className="bg-transparent outline-none text-slate-100 w-full"
          ...
          <div>
            <label className="block text-sm text-slate-400 mb-2">Task Filter</label>
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value)}
              className="w-full px-4 py-2 bg-slate-900 border border-slate-800 rounded-lg text-slate-100 focus:border-amber-500 focus:outline-none"
            >
          ...
          <div className="mt-4 space-y-2">
            {globalSearchResults.map((result, index) => (
              <div key={`${result.type}-${result.name}-${index}`} className="p-3 rounded-lg bg-slate-900 border border-slate-800">
                <p className="text-slate-100 text-sm">{result.name}</p>
                <p className="text-xs text-slate-400">{result.type} • {result.meta}</p>
              </div>
            ))}
          </div>
        )}
          ...
          <Card className="lg:col-span-1">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-slate-100 mt-2">{kpis.openProcurement}</p>
              </div>
        ...
        <Card className="lg:col-span-1">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-slate-100 mt-2">{kpis.unreadNotifications}</p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          ...
          <p className="text-2xl font-bold text-amber-500 mt-2">{formatCurrency(analytics.totalVendorSpend)}</p>
        </Card>
        <Card>
          <p className="text-slate-400 text-sm">Attendance Labor Cost</p>
          <p className="text-2xl font-bold text-emerald-500 mt-2">{formatCurrency(analytics.attendanceLaborCost)}</p>
        </Card>
      </div>
            ...
            {filteredRecentTasks.length === 0 ? (
              ...
              filteredRecentTasks.map((task) => {
                ...
                return (
                  <div
                    key={task.id}
                    className="flex items-center justify-between p-4 bg-slate-800/40 border border-slate-800 rounded-xl hover:bg-slate-900 transition-colors"
                  >
                    <div>
        ...
        <Card title="Inventory Overview">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="p-5 bg-slate-800/40 border border-slate-800 rounded-xl">
              ...
              <p className="text-3xl font-bold text-slate-100 mt-2">{inventory.length}</p>
            </div>
            <div className="p-5 bg-slate-800/40 border border-slate-800 rounded-xl">
            ...
            <div className="p-5 bg-slate-800/40 border border-slate-800 rounded-xl">
              <p className="text-slate-400 text-sm font-medium">Total Inventory Value</p>
              <p className="text-3xl font-bold text-emerald-500 mt-2">
                {formatCurrency(inventory.reduce((sum, i) => sum + (i.current_stock * i.unit_cost), 0))}
              </p>
            </div>
          </div>
        </Card>
