...
export default function LeaveApplication() {
  ...
  const myWorker = useMemo(
    () => workers.find((w) => String(w.user_id) === String(user?.id)),
    [workers, user]
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-100">Leave Applications</h1>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                filterStatus === status
                  ...
                  : 'bg-slate-800/40 text-slate-400 hover:text-slate-50'
              }`}
            >
            ...
            <table className="w-full">
              ...
              <tbody>
                {visibleApplications.map((leave) => (
                  <tr key={leave.id} className="border-b border-slate-800 hover:bg-slate-800/40">
                    {!isWorker && (
                      <td className="py-3 px-4 text-slate-100 font-medium">{getWorkerName(leave.workerId)}</td>
                    )}
                    ...
                    <td className="py-3 px-4 text-slate-400 text-sm max-w-xs truncate">{leave.reason}</td>
                    <td className="py-3 px-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2 bg-slate-800/40 border border-slate-800 rounded-lg text-slate-100 placeholder-slate-500 focus:border-amber-500 focus:outline-none resize-none"
            />
          </div>
