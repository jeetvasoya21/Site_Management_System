...
export default function ProjectTeam() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-100">Project Team Management</h1>
            ...
            <tbody>
              {filteredMembers.map((member) => (
                <tr key={member.id} className="border-b border-slate-800">
                  <td className="py-3 px-4 text-slate-100">{getProjectName(member.projectId)}</td>
                  ...
                  <td className="py-3 px-4">
                    <button className="p-1 text-rose-500 hover:bg-slate-800/40 rounded" onClick={() => removeProjectMember(member.id)}>
                      <Trash2 size={16} />
                    </button>
                  </td>
                </tr>
