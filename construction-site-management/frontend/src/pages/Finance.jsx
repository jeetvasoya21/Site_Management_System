  ...
  if (!canViewFinance) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-100">Finance Analytics</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <div className="space-y-2">
            ...
            <p className="text-2xl font-bold text-amber-500">{formatCurrency(totalBudget)}</p>
          </div>
        </Card>
        <Card>
          <div className="space-y-2">
            ...
            <tbody>
              {projectFinancials.length > 0 ? (
                projectFinancials.map(project => (
                  <tr key={project.id} className="border-b border-slate-800 hover:bg-slate-800/40/50 transition-colors">
                    <td className="py-3 px-4 text-slate-100">{project.project_name}</td>
                    <td className="py-3 px-4 text-slate-100">{formatCurrency(project.budget)}</td>
                    <td className="py-3 px-4 text-slate-100">{formatCurrency(project.totalCost)}</td>
      ...
      <Card title="Cost Category Breakdown">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 bg-slate-800/50 rounded-lg">
            ...
            <p className="text-2xl font-bold text-amber-500">{formatCurrency(laborCosts)}</p>
          ...
          <div className="p-4 bg-slate-800/50 rounded-lg">
            ...
            <p className="text-2xl font-bold text-slate-400">{formatCurrency(materialCosts)}</p>
            <p className="text-xs text-slate-400 mt-2">
          ...
          <div className="p-4 bg-slate-800/50 rounded-lg">
            ...
            <p className="text-2xl font-bold text-blue-500">{formatCurrency(equipmentCosts)}</p>
            <p className="text-xs text-slate-400 mt-2">
          ...
          <div className="p-4 bg-slate-800/50 rounded-lg">
            ...
            <p className="text-2xl font-bold text-purple-500">{formatCurrency(otherCosts)}</p>
            <p className="text-xs text-slate-400 mt-2">
