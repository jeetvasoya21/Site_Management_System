...
const Tasks = () => {
  ...
  const visibleTasks = useMemo(() => {
    if (user?.role === 'Admin' || user?.role === 'Project_Manager') {
      ...
      // Site Engineer sees only their assigned tasks — compare as strings
      return tasks.filter(t => String(t.assigned_to) === String(user?.id));
    }
    return [];
  }, [tasks, user?.role, user?.id]);

  // Group tasks by status — normalize 'In_Progress' (DB) → 'In Progress' for UI
  const tasksByStatus = useMemo(() => {
    const normalize = (s) => s === 'In_Progress' ? 'In Progress' : s;
    return {
      Open: visibleTasks.filter((t) => normalize(t.status) === 'Open'),
      'In Progress': visibleTasks.filter((t) => normalize(t.status) === 'In Progress'),
      Completed: visibleTasks.filter((t) => normalize(t.status) === 'Completed'),
    };
  }, [visibleTasks]);
  ...
  // Get project name — IDs are normalized strings by AppContext
  const getProjectName = (projectId) => {
    return projects.find((p) => String(p.id) === String(projectId))?.project_name || 'Unknown';
  };

  // Get worker name — IDs are normalized strings by AppContext
  const getWorkerName = (workerId) => {
    return workers.find((w) => String(w.id) === String(workerId))?.name || 'Unassigned';
  };
  ...
  const KanbanColumn = ({ status, taskList }) => (
    <div
      ...
      className="bg-slate-800/50 rounded-lg p-4 min-h-96 border-2 border-dashed border-slate-800 hover:border-amber-500/50 transition-colors"
    >
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-semibold text-slate-100">{status}</h3>
        ...
        {taskList.map((task) => {
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <h4 className="text-slate-100 font-medium text-sm flex-1">
                  {task.task_name}
                </h4>
                {canEditTask && (
                  <button
                    onClick={() => handleDelete(task.id)}
                    className="text-rose-500 hover:bg-slate-800/40 p-1 rounded transition-colors flex-shrink-0"
                  >
                    <Trash2 size={14} />
                  </button>
                )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-2 bg-slate-900 border border-slate-800 rounded-lg text-slate-100 focus:outline-none focus:border-amber-500 min-h-28"
              >
                {workers.map((worker) => (
