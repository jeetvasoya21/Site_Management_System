...
export default function WorkerPortal() {
  ...
  const worker = useMemo(() => workers.find((entry) => String(entry.id) === String(selectedWorker)), [selectedWorker, workers]);

  const attendanceHistory = useMemo(() => {
    return attendanceRecords
      .filter((entry) => String(entry.workerId) === String(selectedWorker))
  ...
  const assignedTasks = useMemo(() => {
    ...
    return tasks.filter((task) => taskIds.includes(String(task.id)));
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-100">Worker Portal</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <p className="text-2xl font-bold text-slate-100 mt-2">{formatCurrency(worker?.salary)}</p>
        </Card>
        <Card>
          <p className="text-slate-400 text-sm">Salary from Attendance</p>
          <p className="text-2xl font-bold text-amber-500 mt-2">{formatCurrency(salaryFromAttendance)}</p>
        </Card>
        <Card>
          <p className="text-slate-400 text-sm">Assigned Tasks</p>
          <p className="text-2xl font-bold text-slate-100 mt-2">{assignedTasks.length}</p>
        </Card>
      </div>
            ...
            <tbody>
              {attendanceHistory.map((entry) => (
                <tr key={entry.id} className="border-b border-slate-800">
                  ...
                  <td className="py-3 px-4 text-slate-300">{formatCurrency(entry.labor_cost)}</td>
                </tr>
              ))}
            </tbody>
          </table>
